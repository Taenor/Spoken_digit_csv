...
_YAMNET_LAYER_DEFS = [
    ...
    (_conv,          [3, 3], 2,   4),
    (_separable_conv, [3, 3], 1,   8),
    (_separable_conv, [3, 3], 2,  16),
    (_separable_conv, [3, 3], 1,  16),
    (_separable_conv, [3, 3], 2,  32),
    (_separable_conv, [3, 3], 1,  32),
    (_separable_conv, [3, 3], 2,  64),
...
def yamnet(features, params):
  ...
  gru = layers.Dense(units=20, use_bias=True)(embeddings)
  logits = layers.Dense(units =10, use_bias=True)(gru),
...
def yamnet_frames_model(params):
  ...
  predictions = yamnet(features, params)
  frames_model = Model(
